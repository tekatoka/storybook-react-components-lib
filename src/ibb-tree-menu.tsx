...
import  {
  ...
  LocaleFunction,
  MatchSearchFunction,
} from './elements/ibb-tree-menu/walk';
import { defaultChildren, TreeMenuChildren, TreeMenuItem } from './elements/ibb-tree-menu/ibb-tree-menu-items';
import KeyDown from './elements/ibb-tree-menu/keyDown';

export type ITreeMenuProps = {
...
export class IbbTreeMenu extends React.Component<ITreeMenuProps, TreeMenuState> {
  static defaultProps: ITreeMenuProps = {
    data: {},
  ...
  componentDidUpdate(prevProps: ITreeMenuProps) {
  ...
  generateItems = (): TreeMenuItem[] => {
    const { data, onClickItem, locale, matchSearch, showToggleIcon } = this.props;
    ...
    return items.map(item => {
      ...
      const active = item.key === activeKey || item.active;
      const onClick = () => {
      ...
      return { ...item, focused, active, onClick, toggleNode, showToggleIcon };
    });
  };
